...
export default function AdminSidebar() {
    ...
    return (
        <aside
            className={`fixed left-0 top-0 h-full bg-gradient-to-b from-[#1B5E20] to-[#0D3B10] text-white z-50 transition-all duration-300 ${collapsed ? "w-16" : "w-64"
                } hidden lg:flex flex-col shadow-xl`}
        >
            {/* Logo */}
            <div className={`flex items-center gap-3 p-6 ${collapsed ? "justify-center px-2" : ""}`}>
                    ...
                    return (
                        <Link
                            ...
                            className={`flex items-center gap-3 px-3 py-3 rounded-lg transition-all duration-200 ${isActive
                                    ? "bg-white/10 border-l-3 border-[#F9A825] text-white"
